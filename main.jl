...
############################ Funcoes ##########################################
###############################################################################
pmt(pg,g) =  g[1] + g[2]*pg+ g[3]*pg^2
pgg(pg,f) = f[1]*exp(f[2]*pg)
h(a,b,k_p,k_pusina,k_s,q,V,s) = quartic(a,V) - quartic(b,Q+s) + k_p*q^2 + K_pusina*Q^2 + k_s*q^2
rho(c,q,h) = c[1] + c[2]*q +c[3]*h + c[4]*h*q + c[5]*q^2+ c[6]*h^2 
pst(G,c,q,h) = G*rho(c,q,h)*h*q
quartic(a,x) = sum(a[i]*x^(i-1) for i in eachindex(a)) 
